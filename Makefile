...
PYPATH = $(LIBDIR)/dml/python

OUT_PYFILES     := $(addprefix $(PYPATH)/,$(PYFILES))
OUT_GEN_PYFILES := $(addprefix $(PYPATH)/dml/,$(GEN_PYFILES))

# used later on by refmanual build
PARSER_DEBUGFILES :=	$(PYPATH)/dml12_parser.out \
			$(PYPATH)/dml14_parser.out
...
DMLC_CMD := $(PYTHON) $(PYPATH)/__main__.py
...
$(OUT_PYFILES) $(SCRIPTS): $(PYPATH)/%: $(DMLC_DIR)/py/%
	$(PYCOMPILE) $< $@
...
dml%_parsetab.py $(PYPATH)/dml%_parser.out: \
	...
	$(info Generating $(@F))
	$(PYTHON) $< $(PYPATH) $* dml$*_parsetab $(PYPATH)/dml$*_parser.out
...
$(MPL_LICENSE): $(PYPATH)/%: $(DMLC_DIR)/% | $(PYPATH)
	$(info Copying $*)
	cp $< $@
