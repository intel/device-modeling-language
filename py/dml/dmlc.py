...
def main(argv):
    ...
    if options.strict_int:
        options.breaking_change.append('dml12-fix-int-quirks')
    ...
    dml.globals.enabled_breaking = set(changes.values())
    ...
    dml.globals.enabled_compat = {
        ...
        if bc not in dml.globals.enabled_breaking
    }

    if breaking_changes.enable_WLOGMIXUP not in dml.globals.enabled_breaking:
