...
class remove_port_proxy_ifaces(BreakingChange):
    ...
    interfaces through the dedicated port object. When the change is
    disabled, ports and banks are also redundantly exposed through
    ...
    > [!NOTE]
    > When the change is disabled, legacy interfaces are not created for all
    > ports and banks; no legacy interface is created for constructs that were
    > disallowed in Simics 5, such as banks inside groups.
    '''
...
class remove_port_proxy_attrs(BreakingChange):
    ...
    the bank or port object itself. When the change is disabled,
    ...
    > ports and banks; no proxy attribute is created inside constructs that were
    > disallowed in Simics 5, such as banks inside groups.
    '''
...
@change
class remove_port_obj_param(BreakingChange):
    '''
...
@change
class shared_logs_on_ports(BreakingChange):
    '''
...
@change
class remove_legacy_attributes(BreakingChange):
...
class range_check_method_indices(BreakingChange):
    ...
    '''
    short = "Assert object array indices in method calls"
    required_after = api_7
...
class DML12BreakingChange(BreakingChange):
    def tag(self) -> str:
        return 'dml12-' + self.__class__.__name__.replace('_', '-')


@change
class disable_inline_constants(DML12BreakingChange):
    '''
...
@change
class not_typecheck(DML12BreakingChange):
    '''
...
@change
class remove_misc_quirks(DML12BreakingChange):
    '''
...
@change
class remove_goto(DML12BreakingChange):
    '''
...
@change
class fix_int_quirks(DML12BreakingChange):
    '''
...
@change
class forbid_experimental_vect(BreakingChange):
    '''<a id="forbid_experimental_vect"/>
...
compat_features = {
    ...
    'port_obj_param': remove_port_obj_param,
    'shared_logs_on_device': shared_logs_on_ports,
    'suppress_WLOGMIXUP': enable_WLOGMIXUP,
    'legacy_attributes': remove_legacy_attributes,
    ...
    'dml12_inline': disable_inline_constants,
    'dml12_not': not_typecheck,
    'dml12_misc': remove_misc_quirks,
    'dml12_goto': remove_goto,
    'dml12_int': fix_int_quirks,
    'experimental_vect': forbid_experimental_vect,
