...
def merge_subobj_defs(name, defs, parent):
    ...
    if arrayinfo:
        ...
        for (dim_i, dim) in enumerate(
            ...
            idxvar_asts = (
                ast for ast in idxvar_asts if ast.kind != 'discard')
            try:
                idxvar_ast = next(idxvar_asts)
            except StopIteration:
                idxvar = None
            else:
                assert idxvar_ast.kind == 'variable'
                [idxvar] = idxvar_ast.args
                for other_ast in idxvar_asts:
                    assert other_ast.kind == 'variable'
                    [other] = other_ast.args
                    if other != idxvar:
                        report(EAINCOMP(other_ast.site, idxvar_ast.site, name,
                                        "mismatching index variables"))

            lengths = (
                (eval_arraylen(len_ast, parent_scope), len_ast.site)
                for len_ast in len_asts if len_ast is not None)
            try:
                (length, lensite) = next(lengths)
            except StopIteration:
                ...
                for (other, othersite) in lengths:
                    if other != length:
                        report(EAINCOMP(othersite, lensite, name,
